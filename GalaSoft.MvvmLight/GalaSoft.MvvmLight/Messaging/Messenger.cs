...
namespace GalaSoft.MvvmLight.Messaging
{
    ...
    public class Messenger : IMessenger
    {
        ...
        {
            this.Register(recipient, null, false, action, keepTargetAlive);
        }
        ...
        {
            this.Register(recipient, token, false, action, keepTargetAlive);
        }
        ...
        public virtual void Register<TMessage>(
            ...
            bool keepTargetAlive = false)
        {
            this.Register(recipient, null, receiveDerivedMessagesToo, action, keepTargetAlive);
        }
        ...
        public void RequestCleanup()
        {
            ...
            {
                this.context.Post(_ => cleanupAction(), null);
            }
            else
            {
            ...
            lock (lists)
            {
                ...
                foreach (KeyValuePair<Type, List<WeakActionAndToken>> list in lists)
                {
                    List<WeakActionAndToken> recipientsToRemove = list.Value
                        .Where(item => item.Action == null || !item.Action.IsAlive).ToList();
            ...
            object token)
        {
            ...
            foreach (WeakActionAndToken item in listClone)
            {
                if (item.Action is IExecuteWithObject executeAction && item.Action.IsAlive && item.Action.Target != null
                    && (messageTargetType == null || item.Action.Target.GetType() == messageTargetType
                                                  || messageTargetType.GetTypeInfo().IsAssignableFrom(
                                                      item.Action.Target.GetType().GetTypeInfo()))
        ...
        private static void UnregisterFromLists(object recipient, Dictionary<Type, List<WeakActionAndToken>> lists)
        {
            ...
            lock (lists)
            {
                foreach (IExecuteWithObject weakAction in lists.Keys.SelectMany(
                    messageType => lists[messageType].Select(item => (IExecuteWithObject)item.Action)
                        .Where(weakAction => weakAction != null && recipient == weakAction.Target)))
                {
        ...
        private void SendToTargetOrType<TMessage>(TMessage message, Type messageTargetType, object token)
        {
            ...
            if (this.recipientsOfSubclassesAction != null)
            {
                ...
                List<Type> listClone = this.recipientsOfSubclassesAction.Keys
                    .Take(this.recipientsOfSubclassesAction.Count()).ToList();

                foreach (Type type in listClone)
                {
                    List<WeakActionAndToken> list = null;

                    if (messageType == type || messageType.GetTypeInfo().IsSubclassOf(type)
                                            || type.GetTypeInfo().IsAssignableFrom(messageType.GetTypeInfo()))
                    {
                        lock (this.recipientsOfSubclassesAction)
                        {
                            list = this.recipientsOfSubclassesAction[type]
                                .Take(this.recipientsOfSubclassesAction[type].Count()).ToList();
                        }
                    }
                ...
                lock (this.recipientsStrictAction)
                {
                    if (this.recipientsStrictAction.ContainsKey(messageType))
                    {
                        list = this.recipientsStrictAction[messageType]
                            .Take(this.recipientsStrictAction[messageType].Count()).ToList();
                    }
                }
